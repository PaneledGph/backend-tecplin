...
export class AssistantGeminiController {
  constructor(private readonly assistantEnhanced: AssistantEnhancedService) {}
  ...
  async handleCommand(
    @Req() req,
    @Body() body: { text: string; activeOrderId?: number },
  ) {
    ...
    try {
      ...
      if (!process.env.GEMINI_API_KEY) {
        console.log(
          '⚠️ GEMINI_API_KEY no configurada, usando respuesta de prueba',
        );
        return {
      ...
      if (text.includes('reporte') && text.includes('día')) {
        ...
        return {
          spokenText:
            'Abriendo el reporte del día. Aquí tienes las estadísticas actuales del sistema.',
          confidence: 0.9,
      ...
      const result =
        await this.assistantEnhanced.handleAdvancedCommand(command);

      // Si Gemini no entendió, usar fallback inteligente
      if (
        result.spokenText?.includes('no entendí') ||
        ...
        if (
          ...
          return {
            spokenText:
              'Te muestro el reporte del día con las estadísticas actuales.',
            confidence: 0.8,
            actions: [{ type: 'OPEN_DAILY_REPORT' }],
          };
        }
        ...
        if (
          text.includes('mis órdenes') ||
          text.includes('órdenes asignadas')
        ) {
          return {
        ...
        if (
          text.includes('diagnosticar') ||
          text.includes('problema técnico')
        ) {
          return {
            spokenText:
              'Te ayudo con el diagnóstico técnico. ¿Qué tipo de problema estás enfrentando?',
            confidence: 0.8,
      ...
      if (
        text.includes('diagnosticar') ||
        text.includes('problema técnico') ||
        text.includes('ayuda técnica')
      ) {
        return {
          spokenText:
            'Te ayudo con el diagnóstico técnico. Describe el problema que estás enfrentando.',
          confidence: 0.7,
          actions: [{ type: 'TECH_HELP' }],
        };
      }
      ...
      return {
        spokenText:
          'El asistente está temporalmente fuera de servicio. Intenta usar los botones de la pantalla.',
        error: error.message,
        confidence: 0.1,
      };
    }
