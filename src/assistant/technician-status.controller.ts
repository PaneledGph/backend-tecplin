...
export class TechnicianStatusController {
  ...
  async getTechniciansStatus() {
    try {
      const tecnicos =
        await this.technicianAssignment.getTechniciansWithStatus();
  ...
  async searchTechnicianByName(@Param('name') name: string) {
    try {
      const tecnico =
        await this.technicianAssignment.findTechnicianByName(name);
  ...
  async assignTechnicianToOrder(
    @Body() body: { orderId: number; technicianId?: number; criteria?: string },
  ) {
    try {
