...
export class StorageService {
  ...
  constructor() {
    ...
    this.endpoint =
      process.env.STORAGE_ENDPOINT || (accountId ? `https://${accountId}.r2.cloudflarestorage.com` : undefined);

    if (!this.bucket || !accessKeyId || !secretAccessKey || !this.endpoint) {
      this.logger.warn('⚠️ StorageService no está completamente configurado. Revisa las variables STORAGE_*');
    }
